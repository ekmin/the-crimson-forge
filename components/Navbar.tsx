...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 border-b border-[#393939] transition-all duration-500 ${
    scrolled ? 'backdrop-blur-md bg-white/10' : 'backdrop-blur-none bg-transparent'
  }`}>
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          <Link
            href="/"
            className="text-xl md:text-2xl font-bold tracking-wider text-crimson transition-all duration-200 hover:crimson-glow"
          >
            <EyeBall />
          </Link>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Link
              ...
              className={`relative text-sm tracking-wide transition-all duration-200 group ${
                isActive('/cart') ? 'text-crimson' : 'text-gray-400 hover:text-white'
              }`}
            >
        ...
        {menuOpen && (
          <div className={`md:hidden ${scrolled ? 'bg-trasparent' : 'bg-[#111111]'} border-t border-[#1a1a1a] py-4 space-y-4`}>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
